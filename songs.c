...
static void     song_copy_attrs     ( song * dest, 
                                      song * original );
static gboolean get_mp3_info        ( char * path,
                                      char ** artist,
                                      char ** album,
                                      char ** title );
...
static gboolean read_song_file_type ( char         * path, 
    ...
    case OGG:
        ...
        break;
    case MP3:
        if(get_mp3_info(path, &t_artist, &t_album, &t_title)) {
            if(t_artist)
                *artist = strdup_to_utf8(t_artist);
            if(t_album)
                *album = strdup_to_utf8(t_album);
            if(t_title)
                *title = strdup_to_utf8(t_title);
            return TRUE;
        }       
        break;
    case WAV:
...
static gboolean get_mp3_info ( char * path,
    ...
    *title = NULL;
    ...
    if (!is_mp3) {
        rewind(fp);
        fread(buffer, 1, 3, fp);
        /* A crass way to tell the difference between mp3s, oggs, and wavs,
           at least, is to just look at the first three bytes. */
        if ((buffer[0] == 255) &&
            (buffer[1] >= 250) &&
            (buffer[2] >= 140))
            is_mp3 = TRUE;
